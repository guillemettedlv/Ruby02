...
a = []
journalists.each do |element|
	a << element if all_caps.include?(element[1])
end
puts a.count
...
journalist_order = journalists.sort_by {
    |object|
    object.length
}
puts journalist_order
...
my_hash = Hash[journalists.collect { |element| [element, element.length-1] } ]
puts my_hash
...
puts tab.group_by{|x| x}.map{|k,v| [k,v.count] }